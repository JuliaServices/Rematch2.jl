...
@testset "test for decision automaton optimizations 1" begin
    ...
    @test (Rematch2.@match_count_nodes some_value begin
        Foo(x, 2) => 1
    end) == 5
end
...
@testset "exercise the dumping code for coverage" begin
    io = IOBuffer()
    @test (Rematch2.@match_dumpall io some_value begin
        Foo(x, 2) where !f1(x)            => 1
        Foo(1, y) where !f2(y)            => 2
        Foo(x, y) where !(f1(x) || f2(y)) => 3
        _                                 => 5
    end) == 18
    @test (Rematch2.@match_dump io some_value begin
        Foo(x, 2) where !f1(x)            => 1
        Foo(1, y) where !f2(y)            => 2
        Foo(x, y) where !(f1(x) || f2(y)) => 3
        _                                 => 5
...
@testset "infer positional parameters from Rematch2.match_fieldnames(T) 1" begin
    # struct T207a
    #     x; y; z
    #     T207a(x, y) = new(x, y, x)
    # end
...
@testset "infer positional parameters from Rematch2.match_fieldnames(T) 3" begin
    # struct T207c
    #     x; y; z
    # end
...
@testset "infer positional parameters from Rematch2.match_fieldnames(T) 4" begin
    # struct T207d
    #     x; z; y
...
@testset "diagnostics produced are excellent" begin
    ...
    @testset "infer positional parameters from Rematch2.match_fieldnames(T) 2" begin
        # struct T207b
        #     x; y; z
