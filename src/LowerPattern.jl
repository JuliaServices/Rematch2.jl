...
function code(bound_pattern::BoundFetchFieldPattern, binder::BinderContext)
    tempvar = binder.assignments[bound_pattern]
...
function code(bound_pattern::BoundFetchRangePattern, binder::BinderContext)
    tempvar = binder.assignments[bound_pattern]
...
function code(bound_pattern::BoundFetchLengthPattern, binder::BinderContext)
    tempvar = binder.assignments[bound_pattern]
    :($tempvar = $length($(bound_pattern.input)))
end
function code(bound_pattern::BoundFetchExpressionPattern, binder::BinderContext)
