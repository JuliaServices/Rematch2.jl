...
macro match2_count_states(value, body)
    (top_down_states, state) = build_state_machine(__source__, __module__, value, body)
...
function handle_match2_dump(__source__, __module__, io, value, body, long)
    (top_down_states, state) = build_state_machine(__source__, __module__, value, body)
    esc(:($dumpall($io, $top_down_states, $state, $long)))
end
...
function handle_match2_cases(location::LineNumberNode, mod::Module, value, body)
    (top_down_states, state) = build_state_machine(location, mod, value, body)
